...
const queueRows = [
  { name: 'Maria Santos', status: 'Checked In', color: '#00d4ff' },
  { name: 'James Liu', status: 'Confirmed', color: '#818cf8' },
  { name: 'Patricia Brown', status: 'Completed', color: '#4ade80' },
  { name: 'Robert Garcia', status: 'Billing Ready', color: '#fbbf24' },
]

export default function HeroSection() {
  return (
    <section className="section" style={{ minHeight: '92vh', display: 'flex', alignItems: 'center', paddingTop: '7rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {queueRows.map((row) => (
                <div key={row.name} style={{
                  ...
                  <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.85)' }}>{row.name}</span>
                  <span style={{ fontSize: '0.7rem', fontWeight: 600, color: row.color }}>{row.status}</span>
                </div>
              ))}
            </div>
          </div>
